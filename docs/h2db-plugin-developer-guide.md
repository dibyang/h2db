...
`JdbcUrlPrefixProvider` 用于在数据库打开前注册新的 JDBC URL 前缀，例如把 `jdbc:adb:*` 映射为标准 `jdbc:h2:*` URL。因为 `Driver.acceptsURL()` 和 `Driver.connect()` 发生在 `Database` 创建之前，这类 provider 不能依赖数据库 URL 里的插件加载参数才加载。
