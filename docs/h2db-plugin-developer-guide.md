...
本文档面向插件作者，说明当前插件 SPI 的可用范围、稳定性边界和最小实现方式。更偏使用者视角的 URL 配置和 SQL 示例见 [plugin-usage.md](plugin/plugin-usage.md)。
...
- `JdbcUrlPrefixProvider`：Driver 级 URL 前缀扩展点，用于把 `jdbc:adb:*` 等自定义 URL 映射到 `jdbc:h2:*`。
...
| 层级 | 范围 | 承诺 |
| --- | --- | --- |
| 稳定 SPI | `H2Plugin`、`PluginProvider`、`TableEngineProvider`、`StorageEngineProvider`、`SystemCatalogProvider`、`JdbcUrlPrefixProvider`、`StorageMaintenance`、capability 字符串 | 作为插件入口保持源码兼容，新增能力优先通过默认方法或新 capability 扩展。 |
| 受管迁移 API | `TableEngineContext`、`StorageEngineContext`、`CreateTableData`、`Table` / `Index` 相关内部类型 | ADB 迁移期可用，但升级 H2 小版本前必须跑契约测试，不承诺长期二进制兼容。 |
...
外部插件当前只允许注册 table、storage、system catalog 和 JDBC URL prefix provider。SQL parser、optimizer、wire protocol 等核心扩展点尚未开放。
...
`JdbcUrlPrefixProvider` 用于在数据库打开前注册新的 JDBC URL 前缀，例如把 `jdbc:adb:*` 映射为标准 `jdbc:h2:*` URL。因为 `Driver.acceptsURL()` 和 `Driver.connect()` 发生在 `Database` 创建之前，这类 provider 不能依赖数据库 URL 里的 `PLUGIN_CLASSES` 才加载。

Driver 级插件通过全局插件系统属性、Driver 专用系统属性或 `ServiceLoader` 加载。若同一个插件还提供 table/storage provider，推荐使用全局属性，这样 Driver 早期 URL 解析和 Database 打开后的 provider 注册会使用同一组插件。

| 系统属性 | 含义 |
| --- | --- |
| `h2.pluginClasses` | 逗号分隔的 `H2Plugin` 类名，同时供 Driver 早期加载和 Database 打开后注册。 |
| `h2.pluginPaths` | 逗号分隔的插件 jar 或目录路径，配合 `h2.pluginClasses` 使用。 |
| `h2.pluginServiceLoader` | 设为 `true` 时，Driver 和 Database 都通过 `META-INF/services/org.h2.api.H2Plugin` 自动发现插件。 |
| `h2.driverPluginClasses` | 逗号分隔的 `H2Plugin` 类名，仅供 Driver 早期加载。 |
| `h2.driverPluginPaths` | 逗号分隔的插件 jar 或目录路径，配合 `h2.driverPluginClasses` 使用。 |
| `h2.driverPluginServiceLoader` | 设为 `true` 时，仅 Driver 通过 `META-INF/services/org.h2.api.H2Plugin` 自动发现插件。 |
...
| `Table` / `TableBase` | 受管内部 API | 可用于 ADB 迁移原型；升级 H2 小版本前必须跑契约测试。 |
...
ADB 原型阶段如果发现 `TableEngineContext` 信息不足，按以下优先级处理：
...
`PLUGIN_PATHS` 会为显式插件类创建独立 `URLClassLoader`，并在加载完成后尝试关闭。插件不应依赖加载器在数据库生命周期内保持可变资源打开。

ServiceLoader 加载默认关闭。需要自动发现时，在插件 jar 中加入：
