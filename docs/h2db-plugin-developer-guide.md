...
当前插件机制是数据库打开期的静态加载模型。插件加载完成后 registry 固定，运行中不支持热加载、卸载或在线替换。同一个 plugin id 的多个版本可以并存，但 provider id 仍全局唯一，不能通过版本重载同一个 provider type/id。
...
依赖通过 `PluginDependency` 声明。依赖版本支持和 H2 版本范围相同的最小语法：`*`、精确版本或形如 `[1.0,2.0)` 的区间。加载多个自动发现插件时，H2 会按依赖顺序注册；缺失依赖、版本范围不匹配、重复 plugin id/version 或循环依赖会导致启动失败。

同一个 plugin id 的多个版本可以在同一数据库内并存，但版本必须不同，且 provider id 不能冲突。provider type/id 仍保持全局唯一，因此 SQL 和 URL provider 选择仍是确定的。
...
## Database Lifecycle Provider 约束

数据库生命周期 provider 实现 `DatabaseLifecycleProvider`，provider type 为 `database_lifecycle`。H2 会在数据库关闭过程中触发回调：

- `beforeClose(DatabaseLifecycleContext)`：H2 已进入关闭路径、storage 资源关闭之前触发。
- `afterClose(DatabaseLifecycleContext)`：storage 资源关闭之后、trace system 关闭之前触发。

`DatabaseLifecycleContext` 暴露当前 `Database`、database name、database URL、storage engine id、是否持久化以及是否来自 shutdown hook。该 SPI 面向插件管理的存储引擎清理和诊断，不替代面向应用进度通知的 `DATABASE_EVENT_LISTENER`。

关闭回调失败不会让数据库停留在半关闭状态。H2 会记录第一个 provider 失败，继续完成关闭路径，然后返回包含 provider id、lifecycle event、database name、storage engine id 和 cause 的诊断异常；后续 provider 失败会作为 suppressed exception 附加。

已有 `DATABASE_EVENT_LISTENER` 用户保持兼容。新的 storage engine 插件应优先使用 `DatabaseLifecycleProvider`，不要再通过改写 JDBC URL 注入 `DATABASE_EVENT_LISTENER` 作为插件关闭适配层。
...
这些表只读，用于查看 plugin id、plugin version、display name、provider type/id、来源和 capability。
