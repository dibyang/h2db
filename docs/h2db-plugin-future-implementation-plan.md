...
| `T-PLUGIN-P11-TX-EVENT-COMMIT-01` | JUnit | `h2/src/test-plugin` | `.\gradlew.bat runPluginArchitectureCheck` | P11 | [x] |
| `T-PLUGIN-P11-TX-EVENT-ROLLBACK-01` | JUnit | `h2/src/test-plugin` | `.\gradlew.bat runPluginArchitectureCheck` | P11 | [x] |
| `T-PLUGIN-P11-TX-EVENT-DIAGNOSTIC-01` | JUnit | `h2/src/test-plugin` | `.\gradlew.bat runPluginArchitectureCheck` | P11 | [x] |
| `T-PLUGIN-P11-TX-EVENT-FAILURE-01` | JUnit | `h2/src/test-plugin` | `.\gradlew.bat runPluginArchitectureCheck` | P11 | [x] |
...
- 交付项：
  - 新增 `TransactionEventProvider` 和 `TransactionContext`，作为 commit / rollback 事件的稳定插件 SPI。
  - `SessionLocal` 在真实事务的 commit / rollback 前后触发 provider 回调，并在 provider 抛出运行时异常时返回 provider/event/session 诊断。
  - `PLUGIN_PROVIDERS` / `PLUGIN_CAPABILITIES` 诊断表可展示 transaction provider 与 `transaction.events` capability。
- 当前边界：
  - 不暴露底层 `Transaction` 对象。
  - 不开放 parser、function、auth、optimizer、wire protocol 或完整 session lifecycle。
  - `beforeCommit()` 可阻止 commit；`afterCommit()` 抛错不会回滚已经提交的 H2 事务。
