...
| 阶段 7 | 已完成 | manifest 记录源文件大小和修改时间，`switchToShadow()` 在源文件发生变化时拒绝切换，`T-ONLINE-COMPACT-CATCHUP-WRITES-01` 固定该保守防线。 |
| 阶段 8 | 已完成 | manifest 增加源文件 SHA-256 内容指纹，`switchToShadow()` 同时校验大小、修改时间和内容摘要，降低同大小或时间戳粒度导致的误切换风险。 |
| 阶段 9 | 已完成 | `analyzePreparedShadow()` 明确给出 prepared shadow 是否可切换、版本扫描追平是否可用、是否必须 full copy 降级。结论：当前文件级 shadow compact 阶段尚不能做版本扫描追平。 |
| 阶段 10 | 已完成 | 新增 `refreshShadowIfSourceChanged` 显式降级选项；prepared shadow 过期时可重建 shadow 并执行维护态 full copy，保证新增数据不丢。 |
...
| `T-ONLINE-COMPACT-LONG-TRANSACTION-01` | 活跃读写事务存在时阻止最终切换，维护态下新写请求被拒绝。 |
| `T-ONLINE-COMPACT-TCP-BEHAVIOR-01` | TCP/远程读写请求在维护态下返回明确的 allow、busy、wait 决策。 |
| `T-ONLINE-COMPACT-BACKUP-INTERACTION-01` | backup 与空间回收互斥，不允许交错读写半文件。 |
| `T-ONLINE-COMPACT-LARGE-STORE-01` | 较大膨胀样本仍能收缩且 marker 可读。 |
| `T-ONLINE-COMPACT-SLOW-IO-01` | 慢 IO 延迟路径下仍能完成替换并保留数据。 |
| `T-ONLINE-COMPACT-RESIDUAL-CLEANUP-01` | Windows/失败残留文件可清理，源库继续可读。 |
| `T-ONLINE-COMPACT-VERIFY-FAIL-01` | 校验失败时不替换旧库。 |
| `T-ONLINE-COMPACT-CATCHUP-WRITES-01` | 增量追平未实现前，源库变化时拒绝切换，避免静默丢数据。 |
| `T-ONLINE-COMPACT-CATCHUP-FEASIBILITY-01` | 源库变化时分析结果明确指向 full-copy 降级，而不是不可用的版本扫描追平。 |
| `T-ONLINE-COMPACT-FULL-COPY-FALLBACK-01` | 显式开启降级时，源库变化后重建 shadow 并保留新增数据。 |
| `T-ONLINE-COMPACT-SOURCE-FINGERPRINT-01` | manifest 记录源文件大小、修改时间和 SHA-256 内容指纹。 |
...
- 暂不暴露新 SQL。
- 暂不实现自动调度。
- 不通过普通 Query/SQL 执行后置 hook 触发空间回收；任何自动调度入口必须后续单独评审。
- 暂不实现长期 chunk vacuum。
