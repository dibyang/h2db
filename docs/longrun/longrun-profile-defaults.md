...
本文记录 `h2/src/longrun/resources/longrun-*.properties` 的默认运行时间和周期语义。

## 默认值

| Profile | run.duration | reopen interval | crash interval | crash cycles | 适用场景 |
| --- | --- | --- | --- | --- | --- |
| `longrun-smoke.properties` | 5m | 120s | disabled | 1 | 快速确认工具和 S2 基础链路。 |
| `longrun-reopen.properties` | 1h | 2m | disabled | 1 | 专项 close/open 稳定性，默认约 30 次 reopen。 |
| `longrun-crash.properties` | 30m | 2m | 60s | 15 | 专项 crash/recovery，默认约 15 轮强杀和恢复。 |
| `longrun-nightly.properties` | 12h | 30m | disabled | 1 | 夜间常压长跑。 |
| `longrun-comprehensive.properties` | 12h | 10m | 30m | 12 | 综合常压、reopen、crash/recovery 和 S2 验收。 |
| `longrun-fault-injection.properties` | 30m | 5m | disabled | 1 | copy-based 文件损坏注入专项。 |
| `longrun-soak-30d.properties` | 30d | 2h | 120m | 180 | 专机 30 天 soak。 |
