...
| Full `TestAll ci` runtime | The unpartitioned `runH2TestAllCi` attempt exceeded a 15 minute local timeout | The `memory`, `additional`, and `utils` phases now pass with `MODE=REGULAR`; continue validating the remaining named phases before making the full entrypoint a gate |
...
Currently passing named `TestAll ci` phases: `memory`, `additional`, and `utils`. The remaining phases should continue to be verified and committed one at a time with the same entrypoint.
