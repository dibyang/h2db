...
| 完整 `TestAll ci` 运行时间 | 未拆分的 `runH2TestAllCi` 本地运行超过 15 分钟超时 | `memory`、`additional`、`utils`、`lazy-memory`、`disk`、`disk-additional`、`network-memory` 阶段已在 `MODE=REGULAR` 下通过；继续验证剩余命名阶段后，再恢复完整入口为验收任务 |
...
当前已通过的 `TestAll ci` 命名阶段：`memory`、`additional`、`utils`、`lazy-memory`、`disk`、`disk-additional`、`network-memory`。剩余阶段继续按同一入口逐个验证并提交。
