...
本文档记录在开始 S2 空间在线回收优化之前，当前代码、测试和推进规则是否已经具备开工条件。结论是：插件化前置已经收口，MVStore 空间回收脚手架和专项测试可用，可以进入空间回收优化的详细设计与实现阶段。
...
| `MVStoreSpaceReclamation.switchToShadow()` | 验证 manifest/source fingerprint 后切换 shadow | 可承接 crash-safe publish 方案 |
...
S2 第一轮采用显式触发和低默认风险策略。自动后台回收、阈值调度、周期性维护线程都放到后续阶段，等手动入口、故障恢复和专项测试稳定后再设计。
...
| 风险 | 影响 | 缓解 |
| --- | --- | --- |
| crash-safe publish 语义不清 | 可能导致恢复路径复杂或数据不可用 | S2 设计先拍板 publish 策略，再写代码。 |
| 在线写入与 shadow copy 竞态 | 可能导致切换到过期 shadow | 默认 fingerprint 不一致即拒绝；catch-up 单独设计。 |
...
| 阶段 | 目标 | 交付物 | 验证 |
| --- | --- | --- | --- |
| S2.1 | 明确在线回收决策与统计 | reclaimable size/fill rate/活动事务诊断，不改变文件 | JUnit + `runMvStoreSpaceReclamationCheck` |
| S2.2 | 打通 `vacuumOnline()` 真实维护边界 | 从 `Store.compactFile(50)` 过渡到可诊断的 MVStore 回收流程 | JUnit + MVStore 专项 |
| S2.3 | 实现 shadow prepare 与 gate 策略 | source fingerprint、write gate、long transaction decision | MVStore 专项 + 并发场景 |
| S2.4 | 实现 crash-safe publish | manifest、backup、recover、cleanup 的完整发布语义 | fault harness + recovery tests |
...
| 问题 | 建议默认 |
| --- | --- |
| 是否第一轮支持 crash-safe publish | 支持，但仅限显式触发，且保守保留 backup。 |
| 在线写入期间是否允许 catch-up | 第一轮不做 catch-up；source 变化时拒绝或重新 prepare。 |
...
可以开始空间回收优化。下一步应先写 S2 详细设计，重点拍板 crash-safe publish、在线写入期间的 source 变化策略、长事务 gate、Windows 文件切换语义和测试门禁；随后按 S2.1 到 S2.5 分阶段实现，每阶段完成后本地提交。
