...
config/
  ...
  longrun-comprehensive.properties
  longrun-fault-injection.properties
  longrun-reopen.properties
  longrun-crash.properties
...
cd /opt/h2-longrun
./bin/h2-longrun start -c config/longrun-smoke.properties -d 5m
```
...
cd C:\h2-longrun
bin\h2-longrun.bat -c config\longrun-smoke.properties -d 5m
```
...
```sh
./bin/h2-longrun -c config/longrun-smoke.properties -d 5m
```

Use foreground mode for debugging:

```sh
./bin/h2-longrun run -c config/longrun-smoke.properties -d 5m
```
...
```sh
H2_LONGRUN_INSTANCE=reopen ./bin/h2-longrun start -c config/longrun-reopen.properties -w work/reopen
H2_LONGRUN_INSTANCE=crash ./bin/h2-longrun start -c config/longrun-crash.properties -w work/crash
```
...
The smoke config defaults to `work/smoke`, nightly defaults to `work/nightly`, the comprehensive profile defaults to `work/comprehensive`, the reopen profile defaults to `work/reopen`, the crash profile defaults to `work/crash`, and the 30-day soak config defaults to `work/soak-30d`.
...
```sh
./bin/h2-longrun start -c config/longrun-comprehensive.properties
```
...
```sh
./bin/h2-longrun start -c config/longrun-reopen.properties
```
...
```sh
./bin/h2-longrun start -c config/longrun-crash.properties
```
...
```sh
./bin/h2-longrun start -c config/longrun-fault-injection.properties
```
...
You can override them with `-t, --max-throughput-drop-ratio`, `-f, --max-final-size-gb`, `-p, --max-size-per-million-ops-gb`, `-a, --max-size-amplification`, `-e, --min-reclamation-events`, and `-x, --max-error-lines`.
...
For long runs, throughput-drop analysis uses the 5th percentile throughput from the steady `RUNNING` window, so a single scheduler jitter sample does not turn an otherwise healthy run into WARN. Short runs with fewer samples still use the minimum `RUNNING` sample.
...
```sh
bin/h2-longrun start -c config/longrun-nightly.properties -j /path/to/h2.jar
```
